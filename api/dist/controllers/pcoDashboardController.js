...
const getUpcomingAssignments = async (req, res) => {
    try {
        ...
        const [assignments] = await database_1.pool.query(`
      SELECT 
        c.id as client_id,
        c.company_name,
        c.city,
...
const getRecentReports = async (req, res) => {
    try {
        ...
        const [reports] = await database_1.pool.query(`
      SELECT 
        r.id,
        r.client_id,
        c.company_name,
...
const getDeclinedReports = async (req, res) => {
    try {
        const pcoId = req.user.id;
        const [reports] = await database_1.pool.query(`
      SELECT 
...
exports.getDeclinedReports = getDeclinedReports;
const getStatistics = async (req, res) => {
    ...
    `, [pcoId, period]);
        ...
        const approvalRate = stats.reports_submitted > 0
            ...
            : 0;
        const [topClientResult] = await database_1.pool.query(`
      SELECT 
        c.company_name,
    ...
    `, [pcoId, period]);
        ...
        const [trendResult] = await database_1.pool.query(`
      SELECT 
...
//# sourceMappingURL=pcoDashboardController.js.map